- Reusable Component
iki componenti ortak yere bağlayarak tek componentmiş gibi davranma
```
...
```
 MovieListContainer  ve MyMovieListContainer componentlerini ListContainer adı altında birleştiririz.
###### Birleştirilmiş hali
 ```
 import { useState } from "react";
const movie_list = [
...
function MyMovieListItem({ movie }) {
 ...
 ```
### Api İle Veri Sorgulama

fetch yardımı ile apiye istek atma yapılır. yhemoviedvb api sile yapıldı
...
```
### Component LifeCycle
- Mount(First Render) : Component'in ilk render edilme anı
- Re-Render : Component in tekrar edilmesi.(state, props güncellemesi)
- Unmount : Component in silinme anı
useEffect : Mount yani ilk render edilme anında bu hook kullanılır.
iKİ parametre alır.
useEffect(anonim function, boş dizi ya da başka parametre )
usEeFFECRT İÇERİSİNE BİR KERE SERVİSİ YAZARIZ Kİ SÜREKLİ SERVİSE İSTEK ATIP SONSUZ DÖNGÜYE girmesin sadece sayfa ilk yüklendiği anda çalışsın

### fetch i asenkron ile çağırma
````
...
### Handling Error ve Film Araması yapma
```
import { useState, useEffect } from "react";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    //   setMovies(data.results)
    // })
  }, [query]);  //ilk yüklendiğinde ve search alanındaki query güncellendiğinde useEffect çağrılsın diyoruz
...
```
### Handling  Error ve Film Aramsı Yapma
